...
import { fetchOrders, updateOrderStatus } from "@/lib/ordersData";
...
export default function WorkerJobDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(()=>{fetchOrders().then((items)=>setOrder(items.find((item)=>item.id===params.id)??orders.find((item)=>item.id===params.id)??orders[0])).catch(()=>null);},[params.id]);
  const advance = () => { const nextStage = stage + 1; const status = nextStage === 1 ? "on_the_way" : nextStage === 2 ? "arrived" : nextStage === 3 ? "working" : "completed"; setStage(nextStage); updateOrderStatus(order.id, status).then((updated)=>setOrder("id" in updated ? updated : { ...order, status })).catch(()=>setOrder({ ...order, status })); };
